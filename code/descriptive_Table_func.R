#original code----
desc_table_bygroup_EN <- function(dfr,
                                  vars,
                                  namsvars,
                                  group,
                                  compareGroups=F,
...
alph_desc_table_bygroup_EN <- function(dfr,
                                  vars,
                                  namsvars,
                                  group,
                                  compareGroups=F,
  ...
  for (cur_var in unique(vars) )  {
    ...
    if (is.numeric(dfr[,cur_var])==T){
      ...
      tab1 <- tab1[which(tab1[,2]!="missing"),]
      tab1length <- length(tab1)-4


      tab1<- tab1  %>% relocate("Variable", "Level",.after = last_col())
      tab1sum <- tab1[,order(colnames(tab1[,1:tab1length]),decreasing = FALSE)]

      tab1val<- tab1[, tail(colnames(tab1), 4)]

      tab1 <- cbind(tab1sum, tab1val)

      tab1<- tab1  %>% relocate("Variable", "Level")
      ...
      colnames(tab4) <-  as.list(tab4[1,])

      tab4<-  tab4[,order(colnames(tab4),decreasing = FALSE)]
      ...
      #tab4 <- tab4[,c("Variable","Level",1:(length(tab4)-4),"total","p")]
      tab4<- tab4  %>% relocate("Variable","Level")
